'use strict';
DetailMotamotPostController.$inject = ['$scope', '$rootScope', '$http', '$location', '$routeParams', '$cookies', '$cookieStore', '$filter', '$compile'];
function DetailMotamotPostController($scope, $rootScope, $http, $location, $routeParams, $cookies, $cookieStore, $filter, $compile) {
    ...
    $scope.getCommentListWithPostId = function (postId) {
        $http({
            method: 'GET',
            url: '/api/GetMotamotCommentListWithPostId?postId=' + postId
        }).then(function successCallback(response) {
